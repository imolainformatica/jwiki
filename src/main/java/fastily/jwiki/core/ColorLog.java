...
/**
 * Colors a String and logs it to console. Your terminal must support ASCII
 * escapes for this to work, otherwise the text will not be colored.
 *
 * @author Fastily
 */
class ColorLog {
	 ...
	 * @param color    The color to print the message with. Output will only be
	 *                 colored if this terminal supports it.
	 * @return Logging by matching logLevel.
	 */
	private void log(Wiki wiki, String message, String logLevel, CC color) {
		System.err.printf("%s%n%s: \u001B[3%dm%s: %s\u001B[0m%n", LocalDateTime.now().format(df), logLevel, color.v,
				wiki, message);
	}
	...
	private static enum CC {
		/**
		 * A font color, black, which can be applied to a String if your terminal
		 * supports it.
		 */
		BLACK(0),

		/**
		 * A font color, red, which can be applied to a String if your terminal supports
		 * it.
		 */
		RED(1),

		/**
		 * A font color, green, which can be applied to a String if your terminal
		 * supports it.
		 */
		GREEN(2),

		/**
		 * A font color, yellow, which can be applied to a String if your terminal
		 * supports it.
		 */
		YELLOW(3),

		/**
		 * A font color, blue, which can be applied to a String if your terminal
		 * supports it.
		 */
		BLUE(4),

		/**
		 * A font color, purple, which can be applied to a String if your terminal
		 * supports it.
		 */
		PURPLE(5),

		/**
		 * A font color, cyan, which can be applied to a String if your terminal
		 * supports it.
		 */
		CYAN(6),

		/**
		 * A font color, white, which can be applied to a String if your terminal
		 * supports it.
		 */
		WHITE(7);
